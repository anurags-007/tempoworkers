...
router.get('/worker/:workerId/history', protect, async (req, res) => {
    try {
        if (!validateObjectId(req.params.workerId, res, 'Worker ID')) return;
...
router.get('/employer/:employerId/history', protect, async (req, res) => {
    try {
...
router.post('/:id/rate', protect, async (req, res) => {
    try {
        ...
        // SECURITY: use req.user._id as raterId — never trust client-supplied raterId
        ...
        // Only the job's employer can rate
